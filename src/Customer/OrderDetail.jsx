      ...
      try {
        ...
        if (error.response) {
          ...
          if (error.response.status === 400) {
            toast.error(error.response.data.message || "Lỗi yêu cầu không hợp lệ!", {
              position: "top-right",
              autoClose: 3000,
            });
          } else {
...
<List>
  {order.items.map((product, index) => (
    <ListItem key={index} sx={{ borderBottom: index !== order.items.length - 1 ? "1px solid #ddd" : "none", alignItems: "center", p: 2 }}>
      <ListItemAvatar>
        <Avatar src={product.product_id.image} alt={product.product_id.name} variant="square" sx={{ width: 40, height: 40 }} />
      </ListItemAvatar>

      <ListItemText primary={product.product_id.name} secondary={`Price: $${product.product_id.price}`} sx={{ ml: 2 }} />

      <Button variant="contained" color="primary" sx={{ ml: "auto" }} onClick={() => setOpenIndex(index)}>
        Đánh giá
      </Button>
      ...
      <Dialog open={openIndex === index} onClose={() => setOpenIndex(null)}>
        ...
        <DialogContent>
          ...
          <Rating
            ...
            onChange={(event, newValue) => setRatings({ ...ratings, [index]: newValue })}
          />
          ...
          <TextField
            ...
            rows={3}
            value={reviews[index] || ""}
            onChange={(e) => setReviews({ ...reviews, [index]: e.target.value })}
            sx={{ mt: 2 }}
          />
        </DialogContent>

        <DialogActions>
          <Button onClick={() => setOpenIndex(null)} color="secondary">Hủy</Button>
          <Button
      ...
      <Paper elevation={3} sx={{ p: 3, borderRadius: 3 }}>
        ...
        <List>
          <ListItem>
            <ListItemText primary="Phone Number" secondary={order.shipping_phone} />
          </ListItem>
          <Divider />
          <ListItem>
            <ListItemText primary="Shipping Address" secondary={order.shipping_address} />
          </ListItem>
          <Divider />
          <ListItem>
            <ListItemText primary="Payment Method" secondary={order.order_id?.payment_method || "N/A"} />
          </ListItem>
          <Divider />
          <ListItem>
            <ListItemText primary="Order Date" secondary={new Date(order.createdAt).toLocaleDateString()} />
          </ListItem>
        </List>
      </Paper>
    </Box>
